...
const markerChecks = [
  { label: 'Time', copy: 'How recently was it reported?', color: '#4A90D9' },
  { label: 'Source', copy: 'Community observation or attributed public source?', color: '#2E8B7A' },
  { label: 'Status', copy: 'Is it unverified, corroborated, or official?', color: '#D4A843' },
];

export default function NeighbourhoodWatchGuidePage() {
  return (
    <div className="min-h-dvh bg-[#F8FBFD] text-[#0B1F33]">
      <GuideNav locationLabel="Calgary neighbourhood guide" />
      <main>
        <section className="relative overflow-hidden border-b border-[#B9D5EA] bg-[#E8F3FC]" aria-labelledby="guide-title">
          <div className="absolute inset-y-0 right-0 hidden w-[38%] bg-[#4A90D9] lg:block" aria-hidden="true" />
          <div className="relative mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-8 sm:py-16 lg:grid-cols-[minmax(0,1.04fr)_minmax(24rem,0.96fr)] lg:items-center lg:gap-16 lg:py-20">
            <div className="max-w-3xl">
              <div className="mb-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                <span className="inline-flex items-center gap-2 font-black text-[#176A5D]">
                  <CircleDot size={15} aria-hidden="true" /> Independent Calgary guide
                </span>
                <span className="text-[#67839B]" aria-hidden="true">/</span>
                <span className="font-semibold text-[#52697D]">Reviewed {GUIDE_UPDATED}</span>
              </div>

              <h1 id="guide-title" className="max-w-3xl text-balance font-display text-[clamp(3rem,7vw,5.75rem)] font-black leading-[0.93] tracking-[-0.035em] text-[#0B1F33]">
                Know what’s happening <span className="text-[#286FAF]">around your block.</span>
              </h1>
              <p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-[#40566B] sm:text-xl sm:leading-9">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <a href="/map" className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-[#0B1F33] px-6 font-black text-[#F7FBFF] transition-colors hover:bg-[#174A6E] active:bg-[#092036] ${focusRing}`}>
                  <MapPin size={17} aria-hidden="true" /> Check incidents near me
                </a>
                <a href="/map?report=true" className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border-2 border-[#8DBBDB] bg-[#F8FBFD] px-6 font-black text-[#174A6E] transition-colors hover:border-[#4A90D9] hover:bg-[#D4EAF9] ${focusRing}`}>
                  Sign in to report <ArrowRight size={17} aria-hidden="true" />
                </a>
              </div>

              <p className="mt-7 flex max-w-2xl items-start gap-3 border-y border-[#D4A843] py-4 text-sm font-semibold leading-6 text-[#40566B]">
            ...
            <div className="relative lg:py-5">
              ...
              <figure className="relative overflow-hidden rounded-2xl bg-[#0B1F33] shadow-[0_24px_60px_rgba(11,31,51,0.22)]">
                <img
                ...
                />
                <figcaption className="absolute inset-x-0 bottom-0 bg-[linear-gradient(transparent,rgba(7,23,39,0.94))] px-5 pb-5 pt-20 text-[#F7FBFF] sm:px-6 sm:pb-6">
                  <span className="block text-xs font-black uppercase tracking-[0.16em] text-[#8BC6FF]">Community awareness</span>
                  <span className="mt-1 block max-w-sm text-lg font-black leading-6">Useful context, without speculation or alarm.</span>
                </figcaption>
              </figure>
              <div className="relative -mt-px grid grid-cols-2 overflow-hidden rounded-b-2xl border-x border-b border-[#B9D5EA] bg-[#F8FBFD] sm:absolute sm:-bottom-5 sm:-right-5 sm:w-[21rem] sm:rounded-xl sm:border sm:shadow-lg">
                <a href="tel:911" className={`min-h-16 border-r border-[#E2B8B2] bg-[#FFF4F1] px-4 py-3 transition-colors hover:bg-[#FFE7E2] ${focusRing}`}>
                ...
                <a href="tel:4032661234" className={`min-h-16 px-4 py-3 transition-colors hover:bg-[#E8F3FC] ${focusRing}`}>
                  <span className="block text-[0.68rem] font-black uppercase tracking-wider text-[#52697D]">Not in progress</span>
                  <span className="mt-1 block font-black text-[#174A6E]">403-266-1234</span>
                </a>
              </div>
            </div>
          </div>
        ...
        <section id="understand-the-map" className="scroll-mt-28 bg-[#F8FBFD]" aria-labelledby="near-me-heading">
          <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 md:py-24">
            <div className="grid gap-12 lg:grid-cols-[minmax(0,1.15fr)_minmax(20rem,0.85fr)] lg:gap-20">
              <div>
                <span className="font-mono text-sm font-black text-[#286FAF]">01 / READ THE MAP</span>
                ...
                <div className="mt-7 max-w-3xl space-y-5 text-pretty text-lg leading-8 text-[#52697D]">
              ...
              <aside className="self-start border-t-4 border-[#4A90D9] bg-[#E8F3FC] px-6 py-7 sm:px-8 sm:py-9" aria-label="How to read a map marker">
                <div className="flex items-center gap-3">
                  <Eye className="text-[#286FAF]" size={23} aria-hidden="true" />
                  <h3 className="text-xl font-black tracking-[-0.02em]">Read every marker in context</h3>
                </div>
                <ul className="mt-7">
                  {markerChecks.map((item) => (
                    <li key={item.label} className="grid grid-cols-[4px_5rem_1fr] gap-4 border-t border-[#B9D5EA] py-5 first:border-t-0 first:pt-0 last:pb-0">
                      <span className="h-full min-h-10" style={{ backgroundColor: item.color }} aria-hidden="true" />
                      <strong className="text-sm text-[#174A6E]">{item.label}</strong>
                      <span className="text-sm leading-6 text-[#52697D]">{item.copy}</span>
                    </li>
                  ))}
                </ul>
              </aside>
        ...
        <section id="choose-a-source" className="scroll-mt-28 bg-[#0B1F33] text-[#F7FBFF]" aria-labelledby="source-heading">
          <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 md:py-24">
            <div className="max-w-3xl">
              <span className="font-mono text-sm font-black text-[#8BC6FF]">02 / CHOOSE THE RIGHT SOURCE</span>
              ...
              <p className="mt-5 max-w-2xl text-pretty text-lg leading-8 text-[#C8D8E8]">A community map, official statistics, and emergency reporting each do a different job. Start with what you actually need to know.</p>
            </div>
            ...
            <ol className="mt-10 border-y border-[rgba(255,255,255,0.18)]">
              {GUIDE_COMPARISON.map((row, index) => (
                <li key={row.need} className="grid gap-4 border-b border-[rgba(255,255,255,0.18)] py-6 last:border-b-0 sm:grid-cols-[3rem_minmax(0,1fr)_minmax(12rem,0.55fr)_auto] sm:items-center sm:gap-6 md:py-7">
                  <span className="font-mono text-xl font-black text-[#8BC6FF]" aria-hidden="true">0{index + 1}</span>
                  <p className="max-w-xl text-lg font-black leading-7 text-[#F7FBFF]">{row.need}</p>
                  <p className="text-sm font-semibold leading-6 text-[#AFC5D9]">{row.source}</p>
                  <a
                    href={row.action}
                    rel={row.action.startsWith('http') ? 'external' : undefined}
                    className={`group inline-flex min-h-11 items-center gap-2 font-black text-[#8BC6FF] transition-colors hover:text-[#F7FBFF] ${focusRing}`}
                  >
                    {row.actionLabel}
        ...
        <section className="bg-[#DDF4EE]" aria-labelledby="block-watch-heading">
          <div className="mx-auto grid max-w-7xl lg:grid-cols-2">
            <div className="px-5 py-16 sm:px-8 md:py-24 lg:pr-16">
              <span className="font-mono text-sm font-black text-[#176A5D]">03 / KNOW THE DIFFERENCE</span>
              ...
              <div className="mt-7 max-w-2xl space-y-5 text-pretty text-lg leading-8 text-[#345E57]">
                <p><strong className="text-[#0B1F33]">Block Watch</strong> generally means neighbours organizing on their own block to reduce opportunities for crime, share prevention information, and report suspicious activity through the appropriate channels.</p>
                <p><strong className="text-[#0B1F33]">Calgary Watch</strong> is an independent public map for recent reports across many communities. It is not a Block Watch chapter or a Calgary Police Service program.</p>
              </div>
            </div>

            <div className="bg-[#2E8B7A] px-5 py-16 text-[#F7FBFF] sm:px-8 md:py-24 lg:px-16">
              <div className="flex size-12 items-center justify-center rounded-xl bg-[#0B1F33] text-[#8BC6FF]">
              ...
              <ol className="mt-7 border-y border-[rgba(255,255,255,0.28)]">
                {[
                  ...
                  <li key={step} className="grid grid-cols-[2rem_1fr] gap-4 border-b border-[rgba(255,255,255,0.28)] py-5 last:border-b-0">
                    <Check className="mt-0.5 text-[#F7FBFF]" size={20} strokeWidth={3} aria-hidden="true" />
                    <span className="font-bold leading-6 text-[#F7FBFF]">{step}</span>
                  </li>
                ))}
              </ol>
            </div>
