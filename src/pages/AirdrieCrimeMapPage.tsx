...
export default function AirdrieCrimeMapPage() {
  return (
    <div className="min-h-dvh bg-white text-[#06162F]">
      <GuideNav locationLabel="Airdrie crime map guide" />
      <main>
        <GuideHero
          label="Airdrie community safety guide"
          updated={AIRDRIE_GUIDE_UPDATED}
          title={<>Airdrie crime maps: <span className="text-[#E52C20]">know which map you’re reading.</span></>}
        ...
        <section id="understand-the-map" className="scroll-mt-28 bg-white" aria-labelledby="two-maps-heading">
          <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8 md:py-24">
            <div className="max-w-4xl">
              ...
              <p className="mt-6 max-w-3xl text-pretty text-lg leading-8 text-stone-600">Use Calgary Watch for recent local awareness. Use the City of Airdrie map when the question specifically concerns crime reported to Airdrie RCMP.</p>
            </div>
            <div className="mt-12 grid gap-4 lg:grid-cols-2">
              <article className="rounded-2xl bg-[#06162F] p-6 text-[#F2EFE8] sm:p-8">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex size-11 items-center justify-center rounded-xl bg-[#E52C20] text-[#0B1B14]"><MapPinned size={22} aria-hidden="true" /></div>
                ...
                <p className="mt-4 text-pretty leading-7 text-[#D8CEBC]">Recent community observations and selected public-source incidents. Read each marker’s timestamp, source, and status.</p>
                <a href="/map" className="mt-7 inline-flex min-h-11 items-center rounded-lg bg-white px-4 text-sm font-black text-[#06162F] transition-colors hover:bg-[#EAE3D5] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E52C20]">View community reports</a>
              </article>
              <article className="rounded-2xl bg-[#EAE3D5] p-6 ring-1 ring-[#DCD2C0] sm:p-8">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex size-11 items-center justify-center rounded-xl bg-white text-[#06162F]"><Route size={22} aria-hidden="true" /></div>
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-[#5A5247]">Police-reported crime</span>
                </div>
                <h3 className="mt-8 text-2xl font-black tracking-[-0.02em]">City of Airdrie crime map</h3>
                <p className="mt-4 text-pretty leading-7 text-[#5A5247]">The official starting point for crime reported to Airdrie RCMP. It is separate from Calgary Watch.</p>
                <a href={AIRDRIE_GUIDE_SOURCES[0].url} rel="external" className="mt-7 inline-flex min-h-11 items-center rounded-lg bg-[#06162F] px-4 text-sm font-black text-[#F2EFE8] transition-colors hover:bg-[#06162F] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E52C20]">Open the official map</a>
              </article>
            </div>
            <div className="mt-8 flex max-w-4xl items-start gap-4 rounded-xl bg-[#F7F3EA] p-5 ring-1 ring-stone-200">
              <Eye size={21} className="mt-0.5 shrink-0 text-[#2E8B7A]" aria-hidden="true" />
              <p className="text-pretty leading-7 text-stone-600">Neither map is a public dispatch feed. Neither shows officer locations or confirms that a specific police response is underway.</p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#06162F] text-[#F2EFE8]" aria-labelledby="read-map-heading">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 sm:px-8 md:py-24 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-center lg:gap-20">
            <div>
              ...
              <div className="mt-7 max-w-3xl space-y-5 text-pretty text-lg leading-8 text-[#D8CEBC]">
            ...
            <ol className="overflow-hidden rounded-2xl bg-white text-[#06162F]">
              {[
                ['When', 'When was it reported?'],
                ['Source', 'Who or what supplied it?'],
                ['Action', 'Does it need an official report?'],
              ].map(([label, copy], index) => (
                <li key={label} className="grid grid-cols-[2rem_1fr] gap-4 border-b border-stone-200 p-5 last:border-b-0">
                  <span className="font-black text-[#E52C20]">{index + 1}</span>
                  <span><strong className="block">{label}</strong><span className="mt-1 block text-sm leading-6 text-stone-600">{copy}</span></span>
                </li>
              ))}
            </ol>
          </div>
