...
import { Search, MapPin, Layers, Maximize2, ShieldCheck, AlertCircle, Car, Construction, CloudRain, ShieldQuestion, ShieldAlert, Fuel } from 'lucide-react';
...
export default function Sidebar({ incidents, onIncidentClick, selectedCategory, onCategoryChange, activeIncidentId }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full lg:w-80 bg-slate-950/50 backdrop-blur-2xl border-r border-white/5 overflow-hidden shadow-2xl z-20">
      ...
      <div className="px-4 pt-4 flex items-center gap-3">
        <div className="relative flex-1 group">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
          />
        </div>
        <div className="flex flex-col items-center justify-center bg-blue-500/10 border border-blue-500/20 rounded-xl px-3 py-2 min-w-[56px]">
          <span className="text-[10px] font-bold text-blue-400 uppercase tracking-tighter leading-none">Total</span>
      ...
      <div className="p-4 flex gap-2 overflow-x-auto no-scrollbar shrink-0 border-b border-white/5 scroll-smooth items-center min-w-0">
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={cn(
                'px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-tight transition-all whitespace-nowrap flex items-center gap-2 border shrink-0',
                selectedCategory === cat.id
                  ? 'bg-blue-600 text-white border-blue-400 shadow-lg shadow-blue-500/20'
                  : 'bg-slate-800/50 text-slate-400 border-white/5 hover:bg-slate-800 hover:border-white/10'
              )}
            >
              <Icon size={14} className={cn(selectedCategory === cat.id ? "text-white" : "text-blue-400")} />
              <span>{cat.label}</span>
              <span className={cn(
        ...
        <div className="w-8 shrink-0" /> {/* Larger spacer for end of scroll */}
      </div>

      {/* Stats and Sorting */}
      <div className="px-4 py-3 bg-slate-900/40 border-b border-white/5 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <select 
            ...
            className="bg-slate-900 border border-white/10 rounded-lg px-2 py-1.5 text-[10px] font-bold text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-500/50 cursor-pointer hover:border-white/20 transition-colors"
          >
            ...
            {filteredIncidents.length > 0 ? (
              filteredIncidents.map((incident) => {
                ...
                return (
                  ...
                  >
                    <Card className={cn(
                      "cursor-pointer transition-all duration-300 group border bg-white/[0.03] active:scale-[0.98] relative overflow-hidden light:bg-slate-50 light:border-slate-200 hover:scale-[1.02]",
                      isActive 
                        ? "border-blue-500 bg-blue-500/10 shadow-xl shadow-blue-500/20 ring-1 ring-blue-500/50 animate-active-glow" 
                        : "border-white/5 hover:bg-white/10 hover:border-white/10 hover:shadow-xl hover:shadow-blue-500/5 light:hover:bg-slate-100"
                    )}>
                          ...
                          <div className="flex items-center justify-between mt-3">
                            ...
                            <div className={cn(
                              ...
                              <span className="text-lg font-black tracking-tighter">
                                {incident.neighborhood?.charAt(0) || incident.category?.charAt(0)?.toUpperCase() || '?'}
                              </span>
                            </div>
                          </div>
                        </div>
