...
export function indexableEntities(entities: readonly DiscoveryEntity[]): DiscoveryEntity[] {
  return entities.filter(e => validateEntity(e) && e.status === 'published' && !e.developmentOnly && e.verification === 'source-checked' && e.sources.every(s => s.kind !== 'fixture'));
}
export function buildEntityJsonLd(entity: DiscoveryEntity, origin: string): object {
