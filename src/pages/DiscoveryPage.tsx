...
import { DISCOVERY_SECTIONS, LOCAL_CATEGORIES, matchesPeriod, normalizeSearch, searchEntities } from '../lib/discovery';
...
export default function DiscoveryPage() {
  ...
  let items = all.filter(e => e.kind === section?.kind && (!category || e.categories.includes(category)));
  if (period) items = items.filter(e => e.kind === 'event' ? !e.cancelled && matchesPeriod(e.start, e.end, period) && (!isTonight || Number(new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Edmonton', hour: '2-digit', hourCycle: 'h23' }).format(new Date(e.end))) >= 18) : e.kind === 'market' && discoveryRepository.occurrences().some(o => o.marketId === e.id && !o.cancelled && matchesPeriod(o.start, o.end, period)));
  const groups = searchEntities(all, q);
  const title = missing ? 'This page isn’t here yet.' : search ? 'Find your next Calgary thing.' : category ? `${category[0].toUpperCase()}${category.slice(1)} in Calgary` : isTonight ? 'Tonight in Calgary' : period ? `${period === 'today' ? 'Today' : 'This weekend'} in Calgary` : `${section?.label || 'Discover'} in Calgary`;
  ...
  return <SiteLayout><div className="cw-wrap cw-page">
    ...
    {entity ? <EntityDetail entity={entity} related={all.filter(e => relatedIds.includes(e.id))} /> : <><header className="cw-page-heading"><p className="cw-eyebrow">CalgaryWatch discovery</p><h1>{title}</h1><p className="cw-lead">{missing ? 'The listing may have moved or hasn’t been published.' : 'Good plans begin with a little local knowledge.'}</p></header>
      ...
      <nav className="cw-intents" aria-label="Filter listings"><Link to={`/${root}`} aria-current={!slug ? 'page' : undefined}>All {section?.label.toLowerCase()}</Link>{(root === 'events' || root === 'markets') && <><Link to={`/${root}/this-weekend`}>This weekend</Link>{root === 'events' && <Link to="/events/today">Today</Link>}</>}{root === 'local' && LOCAL_CATEGORIES.map(c => <Link key={c} to={`/local/${c}`} aria-current={category === c ? 'page' : undefined}>{c[0].toUpperCase() + c.slice(1)}</Link>)}</nav>
      ...
      {items.length ? <div className="cw-card-grid">{items.map(e => <DiscoveryCard entity={e} key={e.id} />)}</div> : <EmptyInventory type={section?.label.toLowerCase() || 'discoveries'} />}
      </>}
    </>}
  </div></SiteLayout>;
}
