...
export function SiteHeader() {
  ...
  return <header className="cw-header"><div className="cw-wrap cw-header-inner">
    <button ref={menuButton} className="cw-menu-button cw-menu-button-labeled" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="cw-mobile-nav" aria-label={open ? 'Close menu' : 'Open menu'}>{open ? <X /> : <Menu />}<span>Menu</span></button>
    <Wordmark />
    <div className="cw-header-actions"><Link to="/search" aria-label="Search CalgaryWatch"><Search size={20} /></Link><Link className="cw-button cw-subscribe-button" to="/map?settings=alerts">Subscribe</Link></div>
  </div><nav id="cw-mobile-nav" className="cw-mobile-nav" hidden={!open} aria-label="Main navigation" onKeyDown={e => { if (e.key === 'Escape') { setOpen(false); menuButton.current?.focus(); } }}>{DISCOVERY_SECTIONS.map(s => <NavLink key={s.path} to={s.path}>{s.label}</NavLink>)}<NavLink to="/community" title="CalgaryWatch community: the original homepage">Community</NavLink><Link to="/map">CalgaryWatch Live ↗</Link><Link to="/map?settings=alerts">Account & email preferences</Link></nav></header>;
}
