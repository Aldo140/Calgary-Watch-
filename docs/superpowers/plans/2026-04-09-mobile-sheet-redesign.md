...
**Goal:** Redesign MobileMapSheet to match desktop Sidebar feature parity, fix incident-click-to-popup flow, swap map popup button roles, and remove the redundant top-chrome filter bar.

**Architecture:** Extract `useNeighborhoodPulse` hook for sharing between Sidebar and MobileMapSheet. Fully rewrite MobileMapSheet with sort/filter controls, Neighborhood Pulse, richer cards, and a Report button. Use `flyToWithOffset` (which has an `onComplete` callback) for fly-then-popup. MapPage removes `showMobileFilters` and passes new props.

**Tech Stack:** React 19, TypeScript, vaul (Drawer snap points), motion/react (AnimatePresence), Leaflet via MapRef, Tailwind CSS v4 with `light:` variant, lucide-react icons, date-fns, localStorage persistence.
...
|------|--------|----------------|
| `src/hooks/useNeighborhoodPulse.ts` | **Create** | Shared hook — top 4 neighborhoods with activity in last 2h |
| `src/components/Sidebar.tsx` | **Modify** | Import hook instead of defining inline |
| `src/components/Map.tsx` | **Modify** | Swap popup button roles — Details = primary, Area Intel = secondary |
| `src/pages/MapPage.tsx` | **Modify** | Remove showMobileFilters + Filter button, pass new props to MobileMapSheet |
| `src/components/MobileMapSheet.tsx` | **Modify** | Full redesign — desktop parity |

---

## Task 1: Extract useNeighborhoodPulse hook

**Files:**
- Create: `src/hooks/useNeighborhoodPulse.ts`
- Modify: `src/components/Sidebar.tsx`

- [ ] **Step 1: Create the hook file**

Create `src/hooks/useNeighborhoodPulse.ts` with this exact content:
...
- [ ] **Step 2: Update Sidebar.tsx to import from the hook**

In `src/components/Sidebar.tsx`, find and remove the inline definitions of `RiskLevel`, `NeighborhoodRisk`, `RISK_CONFIG`, and `useNeighborhoodPulse` (lines ~9–41). Replace with a single import:

```ts
import { useNeighborhoodPulse, RISK_CONFIG } from '@/src/hooks/useNeighborhoodPulse';
```

The usage of `useNeighborhoodPulse(incidents)` and `RISK_CONFIG[level]` on lines ~156 and ~350+ stays exactly the same — only the definitions move.

- [ ] **Step 3: Verify TypeScript**

```bash
cd /home/mrotiz14/github-projects/Calgary-Watch--main && npm run lint 2>&1 | grep -v "scripts/"
```

Expected: no errors (only pre-existing `scripts/` error is OK to ignore).

- [ ] **Step 4: Commit**

```bash
git add src/hooks/useNeighborhoodPulse.ts src/components/Sidebar.tsx
git commit -m "refactor: extract useNeighborhoodPulse hook for sharing with MobileMapSheet"
```

---

## Task 2: Swap map popup button roles

**Files:**
- Modify: `src/components/Map.tsx` (~lines 163–174)

The current popup has "Area Intelligence" as the primary blue button and "Details" as the small ghost button. We swap them: Details becomes primary blue, Area Intel becomes secondary ghost.

- [ ] **Step 1: Find the button construction block**

In `src/components/Map.tsx`, find the block that creates `learnMore` and `viewDetails` buttons (around lines 163–174). It looks like this:
...
- [ ] **Step 2: Swap the button styles and append order**
...
Note: class names `view-details-btn` and `learn-more-btn` are unchanged — the click handler in Map.tsx reads these class names and they must stay the same.
...
git commit -m "fix: make Details the primary button in map pin popup"
```

---

## Task 3: MapPage cleanup — remove showMobileFilters, add new sheet props

**Files:**
- Modify: `src/pages/MapPage.tsx`

- [ ] **Step 1: Remove showMobileFilters state**

Find and delete line ~355:
```ts
const [showMobileFilters, setShowMobileFilters] = useState(false);
```

- [ ] **Step 2: Remove the Filter button from top chrome**

Find and delete the Filter button (lines ~937–949):
```tsx
<button
  type="button"
...
- [ ] **Step 3: Remove the collapsible category row AnimatePresence block**

Find and delete from the comment through the closing tag (lines ~952–982):
```tsx
...
- [ ] **Step 4: Remove unused Filter import**

In the lucide-react import at the top of MapPage.tsx, remove `Filter` from the import list (only if it's not used elsewhere — grep first):

```bash
grep -n "Filter" src/pages/MapPage.tsx
```

If the only remaining uses are the ones you just deleted, remove `Filter` from the import.

- [ ] **Step 5: Add new props to MobileMapSheet usage**

Find the `<MobileMapSheet ... />` block (~lines 886–900) and add the new props:
...
- [ ] **Step 6: Verify TypeScript (will fail until Task 4 updates MobileMapSheet props)**

```bash
cd /home/mrotiz14/github-projects/Calgary-Watch--main && npm run lint 2>&1 | grep -v "scripts/"
```

Expected: TypeScript errors about unknown props on MobileMapSheet — this is expected. They will be resolved in Task 4.

- [ ] **Step 7: Commit**

```bash
git add src/pages/MapPage.tsx
git commit -m "refactor: remove showMobileFilters top chrome, wire new MobileMapSheet props"
```

---

## Task 4: Full MobileMapSheet redesign

**Files:**
- Modify: `src/components/MobileMapSheet.tsx`

This is a complete rewrite. The entire file content is replaced.

**Key facts about the existing codebase you must know:**
- `CATEGORY_ICONS` is imported from `@/src/types` — maps `IncidentCategory` → Lucide component
- `STATUS_ICONS` is also in `@/src/types` — maps `verified_status` → Lucide component
- `MapRef` is exported from `@/src/components/Map` — has `flyToWithOffset(lat, lng, options?: { zoom?, offsetX?, offsetY?, onComplete? })` and `showPopup(incident)`
- `flyTo` does NOT have an `onComplete` callback — use `flyToWithOffset` instead
- `useNeighborhoodPulse`, `RISK_CONFIG`, `NeighborhoodRisk` are imported from `@/src/hooks/useNeighborhoodPulse`
- `cn` is imported from `@/src/lib/utils`
- Light mode uses `light:` variant (defined as `@variant light (&:where(.light, .light *))`) — not Tailwind's built-in `dark:`
- `SnapPoint` and `SNAP_POINTS` are already exported from this file

- [ ] **Step 1: Replace the full file content**

Write `src/components/MobileMapSheet.tsx` with the following complete content:
            ...
            <div className="flex flex-col flex-1 min-h-0">
                        ...
                        className={cn(
                          ...
                          isSelected
                            ...
                            : dark
                              ...
                              : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-100 light:bg-white light:text-slate-700 light:border-slate-300',
                        )}
                      >
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                      ...
                      className={cn(
                        ...
                        verifiedOnly
                          ...
                          : dark
                            ...
                            : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-50 light:bg-white light:text-slate-700 light:border-slate-300',
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        'px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border transition-all',
                        recentOnly
                          ? dark ? 'bg-blue-500/20 text-blue-400 border-blue-500/40' : 'bg-slate-900 text-white border-slate-900 light:bg-slate-900 light:text-white light:border-slate-900'
                          : dark
                            ? 'bg-white/5 text-slate-400 border-white/10 hover:bg-white/10'
                            : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-50 light:bg-white light:text-slate-700 light:border-slate-300',
                      )}
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          dark
                            ...
                            : 'border-slate-300 text-slate-700 hover:bg-slate-50 light:border-slate-300 light:text-slate-700',
                        )}
                      >
                        Clear Filters
                      </button>
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            dark
                              ...
                              : 'bg-blue-50 hover:bg-blue-100 border-blue-200 light:bg-blue-50 light:border-blue-200',
                          )}
                        >
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              cfg.bg,
                              dark ? 'border-white/5' : 'border-slate-200 light:border-slate-200',
                            )}
                      ...
                      return (
                        <motion.button
                          ...
                          className={cn(
                            ...
                            dark ? 'bg-white/[0.04]' : 'bg-white light:bg-white',
                            isEmergency
                              ...
                              : isActive
                                ...
                                : dark
                                  ...
                                  : 'border-slate-200 hover:bg-slate-50 light:border-slate-200 light:hover:bg-slate-50',
                          )}
                        >
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      dark
                        ...
                        : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50 light:border-slate-300 light:bg-white light:text-slate-700',
                    )}
                  >
              ...
              {isExpanded && (
                <div className={cn(
                  ...
                  dark ? 'border-white/5 bg-slate-950/60' : 'border-slate-200 bg-white light:border-slate-200 light:bg-white',
                )}>
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      dark
                        ...
                        : 'bg-slate-900 hover:bg-slate-800 text-white light:bg-slate-900 light:text-white',
                    )}
                  >
                    <Plus size={14} />
                    Report an Incident
...
Expected: no errors. If you see `Property 'onIncidentClick' does not exist`, it was intentionally removed from the destructuring (it's still in the interface for MapPage compatibility but not used inside the sheet). If TypeScript complains about unused destructured props, remove `onIncidentClick` from the destructuring params while keeping it in the interface.

- [ ] **Step 3: Verify build**

```bash
cd /home/mrotiz14/github-projects/Calgary-Watch--main && npm run build 2>&1 | tail -20
```

Expected: build completes, no errors (bundle size warnings are OK).

- [ ] **Step 4: Commit**

```bash
git add src/components/MobileMapSheet.tsx
git commit -m "feat: redesign MobileMapSheet with desktop parity — sort, filters, pulse, rich cards, report button"
```
...
|-----------------|------|
| Brand gradient stripe (collapsed + open) | Task 4 |
| Report button in collapsed bar + expanded footer | Task 4 |
| Remove showMobileFilters / Filter button / category row | Task 3 |
| Search + total count badge | Task 4 |
| Category chips with icons + counts | Task 4 |
| Sort select (newest/oldest/verified) persisted to localStorage | Task 4 |
| Verified Only + Recent 2h toggles persisted | Task 4 |
| Clear Filters button when active | Task 4 |
| Neighborhood Pulse (expanded only, no search) | Task 4 |
| Pulse tap → flyToWithOffset + collapse | Task 4 |
| Richer incident cards (description, source badge, status, avatar) | Task 4 |
| Incident tap → flyToWithOffset(onComplete → showPopup) | Task 4 |
| Empty state (with/without active filters) | Task 4 |
| Load More button (expanded only) | Task 4 |
| Peek "Show all" hint | Task 4 |
| activeIncidentId scroll-into-view | Task 4 |
| Extract useNeighborhoodPulse hook | Task 1 |
| Sidebar imports hook | Task 1 |
| Map popup: Details = primary blue, Area Intel = ghost | Task 2 |
| MapPage passes new props (hasMore, onLoadMore, onReportPress, activeIncidentId) | Task 3 |
